...
# CONFIG
# =========================================================
PACKAGE_DIR = "Manual31_Training_Package"
MANIFEST_PATH = os.path.join(PACKAGE_DIR, "manifest.csv")

OUTPUT_DIR = "manual31_training_outputs"
...
class ManualSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        joint = str(row[self.joint_col]) if self.joint_col is not None else f"sample_{idx:02d}"
        ...
        if idx < 2:
            ...
            print("mask unique after resize+binarize:", torch.unique(mask))
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(images.size(0)):
                ...
                base = f"sample_{int(indices[i]):02d}_{str(joints[i]).replace('/', '_').replace(' ', '_')}"
...
def main():
    ...
    dataset = ManualSegmentationDataset(
    ...
    ckpt_path = os.path.join(CHECKPOINT_DIR, "unet_manual31.pt")
    ...
    plt.title("Manual31 Sanity-Check Training Loss")
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.title("Manual31 Sanity-Check Training Dice")
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.title("Manual31 Sanity-Check Training IoU")
    plt.grid(True)
    plt.tight_layout()
    ...
    summary = {
        "dataset_name": "Manual31_Training_Package",
        ...
        "device": DEVICE,
        "notes": "Sanity-check run on 31-pair manual-mask subset."
    }
